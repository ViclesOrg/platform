...
export default class carInfo extends builder.Component {
  ...
  #buildConditionsObject() {
    ...
    conditions.model_year = (field) => {
      return (
        parseInt(field.value) >= 1940 &&
        parseInt(field.value) <= new Date().getFullYear() + 1
      );
    };
    ...
    builder.brdige(
      ...
      (data) => {
        ...
        if (data.hasOwnProperty("code") && data.code === -1) {
          ...
          const toast = new Toast(
            "Erreur lors de la mise à jours de la voiture",
            3000,
            "error_toast",
          );
          toast.show();
  ...
  create() {
    let win = new _window(
      ...
      car_brand_model_container = builder.block(
        null,
        "v_vicles_car_brand_model_container",
        [],
      ),
      plate_number = new IconField(
        '<i class="ri-hashtag"></i>',
        "Matricule",
        "text",
        ["Champs obligatoire", "Format: 12345-A-123"],
        "v_vicles_carInfo_inputs",
        "plate",
      ),
      model_year = new IconField(
        '<i class="ri-calendar-schedule-line"></i>',
        "Année du model",
        "number",
        [
          "Champs obligatoire",
          "Entre 1940 et " + (new Date().getFullYear() + 1),
        ],
        "v_vicles_carInfo_inputs",
        "model_year",
      ),
      price = new IconField(
        ...
        "Prix par jour",
        "number",
        ["Champs obligatoire", "Entre 100 et 50000"],
        "v_vicles_carInfo_inputs",
        "price",
      ),
      seats = new IconField(
        ...
        "Nombre de places",
        "number",
        ["Champs obligatoire", "Entre entre 1 et 10"],
        "v_vicles_carInfo_inputs",
        "seats",
      ),
      miles = new IconField(
        ...
        "Kilométrage",
        "number",
        ["Champs obligatoire", "Entre 0 et 1000000"],
        "v_vicles_carInfo_inputs",
        "miles",
      ),
      trunc = new IconField(
        ...
        "Volume du  coffre",
        "number",
        ["Champs obligatoire", "Entre 5 et 1000"],
        "v_vicles_carInfo_inputs",
        "trunc",
      ),
      gear = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [
          { id: "-1", name: "Boite à vitesse" },
        ...
        (item) => {
          ...
          if (builder.isArabic(item.text)) lang.style.direction = "rtl";

          container.setAttribute("itemValue", item.id);
          return container;
        },
      ),
      ac = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [
      ...
      car_gear_ac_container = builder.block(
        null,
        "v_vicles_car_brand_model_container",
        [gear.getHTML(), ac.getHTML()],
      ),
      fuel = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [
      ...
      cities = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [{ id: "-1", name: "Sélectionner une ville" }],
        (item) => {
          let city = builder.label("v_vicles_dropdown", item.name),
            container = builder.block(null, "dropDownItem", [city]);
      ...
      car_fuel_container = builder.block(
        null,
        "v_vicles_car_brand_model_container",
        [fuel.getHTML(), cities.getHTML()],
      ),
      car_information = builder.block(null, "v_vicles_carInfo_left", [
        car_brand_model_container,
        plate_number.getHTML(),
        model_year.getHTML(),
        price.getHTML(),
        seats.getHTML(),
        miles.getHTML(),
        trunc.getHTML(),
        car_gear_ac_container,
        car_fuel_container,
      ]),
      drop_images = builder.block(null, "v_vicles_car_drop_images", [
        builder.button(
          null,
          "v_vicles_drop_image_icon",
          null,
          '<i class="ri-image-add-line"></i>',
        ),
        builder.label(
          "v_vicles_drop_image_text",
          "Déposez ou cliquez pour télécharger l'image de couverture",
          "v_vicles_drop_image_text",
        ),
      ]),
      fileInput = builder.textBox(
        null,
        "Déposez ou cliquez pour télécharger l'image de couverture",
        "file",
        "v_vicles_drop_file_input",
      ),
      small_image_drop = builder.block(null, "v_vicles_car_small_images", [
        builder.button(
          null,
          "v_vicles_drop_image_icon",
          null,
      ...
      small_image_miniature1 = builder.block(
        null,
        "v_vicles_car_small_miniature",
        [],
      ),
      small_image_miniature2 = builder.block(
        null,
        "v_vicles_car_small_miniature",
        [],
      ),
      fileInput_small_image = builder.textBox(
        null,
        "Déposez ou cliquez pour télécharger l'image de couverture",
        "file",
        "v_vicles_drop_file_input",
      ),
      scrollable_car_images = builder.block(
        null,
        "v_vicles_car_scrollable_images",
        [
          small_image_drop,
          small_image_miniature1,
          small_image_miniature2,
          fileInput_small_image,
        ],
      ),
      add_car_button = builder.button(
        null,
        "v_small_button_main",
        this.#mode === "create" ? "Ajouter" : "Modifer",
        null,
      ),
      cancel_add_car_button = builder.button(
        null,
        "v_small_button_black",
        "Annuler",
        null,
      ),
      buttons_container = builder.block(null, "v_vicles_carInfo_buttons", [
        add_car_button,
        cancel_add_car_button,
      ]),
      car_images = builder.block(null, "v_vicles_carInfo_right", [
        drop_images,
        fileInput,
        scrollable_car_images,
        buttons_container,
      ]),
      zone = win.appZone,
      model_id = -1,
      ...
      cover = null,
      images = [],
      car_brand = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [{ id: "-1", name: "Sélectionner une marque" }],
        (item) => {
          let brand = builder.label("v_vicles_dropdown", item.name),
            container = builder.block(null, "dropDownItem", [brand]);
      ...
      car_model = new builder.Dropdown(
        null,
        "v_vicles_fields",
        '<i class="ri-arrow-drop-down-line"></i>',
        [{ id: "-1", name: "Sélectionner un model" }],
        (item) => {
          let brand = builder.label("v_vicles_dropdown", item.name),
            container = builder.block(null, "dropDownItem", [brand]);
    ...
    scrollable_car_images.addEventListener("click", () =>
      fileInput_small_image.click(),
    );
    ...
    function handleFile(file) {
      if (file && file.type.startsWith("image/")) {
        ...
        reader.readAsDataURL(file);
      } else drop_images.classList.add("circle_over_danger");
    }
    cancel_add_car_button.onclick = () => {
      if (this.#mode === "edit")
        this.component.parentNode.removeChild(this.component);
      else if (this.#mode === "create") history.back();
    };
    ...
    const car_info_validator = new builder.Validator(
      car_information,
      (i, t) => {
    ...
    add_car_button.onclick = () => {
      ...
      if (small_image_id === -1) {
        small_image_drop.classList.add("circle_over_danger");
        valid = false;
      } else small_image_drop.classList.remove("circle_over_danger");
      if (fileInput.files.length === 0 && this.#mode === "create") {
        drop_images.classList.add("circle_over_danger");
        valid = false;
      } else drop_images.classList.remove("circle_over_danger");
      if (model_id === -1) {
      ...
      if (gear_id === -1) {
        ...
        valid = false;
      } else gear.getHTML().classList.remove("circle_over_danger");
      if (ac_id === -1) {
        ac.getHTML().classList.add("circle_over_danger");
        valid = false;
      } else ac.getHTML().classList.remove("circle_over_danger");
      if (fuel_id === -1) {
        fuel.getHTML().classList.add("circle_over_danger");
        valid = false;
      } else fuel.getHTML().classList.remove("circle_over_danger");
      if (city_id === -1) {
        cities.getHTML().classList.add("circle_over_danger");
        valid = false;
      } else cities.getHTML().classList.remove("circle_over_danger");
      if (!car_info_validator.validate()) valid = false;
      if (valid) {
        ...
        fd.append(
          ...
          images.length === 0 &&
            this.#car !== undefined &&
            this.#car.images.length > 0
            ? JSON.stringify(this.#car.images)
        ...
        if (this.#mode === "edit") fd.append("id", this.#car.id);

        if (this.#mode === "create") this.#createCar(fd, add_car_button);
        else if (this.#mode === "edit") this.#editeCar(fd, add_car_button);
      } else {
        const toast = new Toast(
    ...
    builder.brdige(
      ...
      (data) => {
        ...
        cities.onChange = (city, name) => {
          city_id = city;
        };

        if (this.#mode === "edit") {
          cities.setValue(this.#car.city);
        }
      },
      () => {},
    );
    ...
    builder.brdige(
      ...
      (data) => {
        ...
        car_brand.updateItems([
          { id: "-1", name: "Sélectionner une marque" },
          ...data,
        ]);

        car_brand_model_container.append(
          car_brand.getHTML(),
          car_model.getHTML(),
        );
